...
function initCOSimulator() {
  ...
  // Voice confirmation button click ("Mən Yaxşıyam") - Immediate safe transition without annoying popups
  ...
  voiceConfirmBtn?.addEventListener('click', (e) => {
    ...
    const isEn = lang === 'en';
    const safeMsg = isRu
      ? 'Безопасность подтверждена: тревога отключена, статус передан семье.'
      : (isEn 
        ? 'Safety confirmed: alarm deactivated, family status updated.'
        : 'Təhlükəsizlik statusu təsdiqləndi: siqnal dayandırıldı, ailəyə status ötürüldü.');

    showToast(safeMsg, 3200);
  });
...
/* ==========================================================================
   Language Switcher (Instant Client-side Toggle & Default Azerbaijani)
   ========================================================================== */
function initLanguageSwitcher() {
  ...
  const routeLang = path.split('/').filter(Boolean)[0];
  const savedLang = localStorage.getItem('airsen_user_lang_set');
  const initialLang = ['az', 'ru', 'en'].includes(routeLang)
    ? routeLang
    : (['az', 'ru', 'en'].includes(savedLang) ? savedLang : currentLang);
  ...
  localStorage.setItem('airsen_user_lang_set', initialLang);
  langButtons.forEach(b => b.classList.toggle('active', b.dataset.lang === initialLang));
...
function initScrollspy() {
  ...
  // Collect sections that correspond to navigation links
  const navSectionIds = ['problem', 'how-it-works', 'features', 'pricing', 'team', 'investors', 'reviews', 'contact'];
  ...
  allNavLinks.forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      if (targetEl) {
        ...
        // Immediately update active status on click
        if (targetId !== '#hero') {
          setActive(targetId);
  ...
  const handleScrollspy = () => {
    ...
    // 1. Bottom of page edge case -> activate the last section (#contact)
    if (scrollY + windowHeight >= docHeight - 80) {
      setActive('#contact');
      return;
    }

    // 2. Top of page edge case -> before #problem section
    const problemEl = document.getElementById('problem');
    if (scrollY < 120 || (problemEl && problemEl.getBoundingClientRect().top > 250)) {
      setActive('');
      return;
    }

    // 3. Find active section based on viewport position
    let activeId = '';
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      // If section top is above headerOffset and bottom is still below headerOffset
    ...
    // Fallback: If no section matched strict bounds but user is deep in the page
    if (!activeId) {
      for (let i = sections.length - 1; i >= 0; i--) {
        const sec = sections[i];
        if (sec.getBoundingClientRect().top <= headerOffset + 50) {
          activeId = `#${sec.id}`;
          break;
        }
      }
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!res.ok || data.success !== true) {
        throw new Error(data.message || 'Beta registration failed');
      }

      closeModal();
      form.reset();

      // Multilingual Formal Modal Content (Zero smileys, pure professional language)
      let alertTitle = 'Müraciətiniz qeydə alındı';
      let alertHtml = `
        <div style="text-align: left; font-size: 14px; line-height: 1.6; color: #E2E8F0; margin-top: 10px;">
          <p style="margin-bottom: 12px;"><strong>${email}</strong> e-poçt ünvanı bildiriş siyahısına uğurla əlavə edildi.</p>
          <div style="background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 14px; margin-bottom: 8px;">
            <p style="margin: 0 0 6px 0; font-size: 13.5px; color: #FFFFFF; font-weight: 600;">Tətbiq Google Play və App Store-da yayımlandıqda sizə bildiriş göndəriləcək və 3 aylıq Premium istifadə hüququ aktivləşdiriləcəkdir.</p>
            <p style="margin: 0; font-size: 12px; color: var(--text-muted, #9CA3AF);">Heç bir bank kartı tələb olunmur və ödənişsizdir.</p>
          </div>
        </div>
      `;
      let btnText = 'Bağla';
      ...
      if (lang === 'ru') {
        alertTitle = 'Заявка успешно принята';
        alertHtml = `
          <div style="text-align: left; font-size: 14px; line-height: 1.6; color: #E2E8F0; margin-top: 10px;">
            <p style="margin-bottom: 12px;">Электронный адрес <strong>${email}</strong> добавлен в список ожидания.</p>
            <div style="background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 14px; margin-bottom: 8px;">
              <p style="margin: 0 0 6px 0; font-size: 13.5px; color: #FFFFFF; font-weight: 600;">При официальном релизе приложения вам будет направлено уведомление и предоставлен 3-месячный бесплатный доступ к Premium-функциям.</p>
              <p style="margin: 0; font-size: 12px; color: var(--text-muted, #9CA3AF);">Банковские реквизиты не требуются.</p>
            </div>
          </div>
        `;
        btnText = 'Закрыть';
      } else if (lang === 'en') {
        alertTitle = 'Request Submitted Successfully';
        alertHtml = `
          <div style="text-align: left; font-size: 14px; line-height: 1.6; color: #E2E8F0; margin-top: 10px;">
            <p style="margin-bottom: 12px;"><strong>${email}</strong> has been added to the release notification list.</p>
            <div style="background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 14px; margin-bottom: 8px;">
              <p style="margin: 0 0 6px 0; font-size: 13.5px; color: #FFFFFF; font-weight: 600;">You will receive an official notification and a 3-month free trial when the application is launched.</p>
              <p style="margin: 0; font-size: 12px; color: var(--text-muted, #9CA3AF);">No payment information is required.</p>
            </div>
          </div>
        `;
        btnText = 'Close';
      ...
      console.error(err);
      if (typeof Swal !== 'undefined') {
        const errTitle = lang === 'ru' ? 'Ошибка' : (lang === 'en' ? 'Error' : 'Xəta');
        const errText = lang === 'ru' 
          ? 'Произошла ошибка при регистрации. Пожалуйста, повторите попытку позже.'
          : (lang === 'en' ? 'An error occurred during registration. Please try again later.' : 'Qeydiyyat zamanı xəta baş verdi. Zəhmət olmasa bir qədər sonra yenidən cəhd edin.');
        const errBtn = lang === 'ru' ? 'Закрыть' : (lang === 'en' ? 'Close' : 'Bağla');
